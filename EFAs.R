...
IAT_Items[,1:20]<-lapply(IAT_Items[,1:20], as.numeric)
nfactors(IAT_Items)
...
paran(IAT_Items[2:21], iterations = 10000, centile = 95, quietly = FALSE, 
      status = TRUE, all = TRUE, cfa = TRUE, graph = TRUE, color = TRUE, 
      col = c("black", "red", "blue"), lty = c(1, 2, 3), lwd = 1, legend = TRUE, 
      file = "", width = 640, height = 640, grdevice = "png", seed = 0)

paran(PCIAT_Items[2:21], iterations = 10000, centile = 95, quietly = FALSE, 
      status = TRUE, all = TRUE, cfa = TRUE, graph = TRUE, color = TRUE, 
      col = c("black", "red", "blue"), lty = c(1, 2, 3), lwd = 1, legend = TRUE, 
      file = "", width = 640, height = 640, grdevice = "png", seed = 0)
...
data.fa1 <- factanal(IAT_Items[2:21], factors = 7, rotation = "varimax")
data.fa1

#data.pca <- princomp(PCIAT_Items[2:21])
#summary(data.pca)
#plot(data.pca, main = "Scree Plot")
data.fa1 <- factanal(PCIAT_Items[2:21], factors = 7, rotation = "varimax")
data.fa1
