...
IAT[,2:21]<-lapply(IAT[,2:21], ordered) 
IAT_Items[,2:21]<-lapply(IAT_Items[,2:21], ordered)
PCIAT[,2:21]<-lapply(PCIAT[,2:21], ordered) 
PCIAT_Items[,2:21]<-lapply(PCIAT_Items[,2:21], ordered)


# cool code
library(tidyverse)
IAT_Items<-select(IAT_Items, -URSI)
...
fit_IAT <- cfa(IAT_confirmatory_1f, data = IAT_Items, std.lv=TRUE)
...
fit_PCIAT <- cfa(PCIAT_confirmatory_1f, data = PCIAT_Items, std.lv=TRUE)
...
fit <- cfa(PCIAT_confirmatory_1, data = PCIAT, std.lv=TRUE)
summary (fit, fit.measures=TRUE)
...
F6 =~ IAT_04 + IAT_03'
fit <- cfa(IAT_confirmatory_2, data = IAT, std.lv=TRUE)
...
fit <- cfa(PCIAT_confirmatory_2, data = PCIAT, std.lv=TRUE)
summary (fit, fit.measures=TRUE) # std.lv=TRUE
